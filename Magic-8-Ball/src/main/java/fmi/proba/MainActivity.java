...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        co.setText("You are using me for : " + totalCount + " time");
        co.setVisibility(View.VISIBLE);
        co.postDelayed(new Runnable() {
            public void run() {
    ...
    private boolean isShakeEnough(float x, float y, float z){
        ...
        force += Math.pow((x - lastY)/SensorManager.GRAVITY_EARTH, 2.0);
        force += Math.pow((x - lastZ)/SensorManager.GRAVITY_EARTH, 2.0);
